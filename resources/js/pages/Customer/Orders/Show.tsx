...
export default function CustomerOrderShow({ order }: Props) {
    const paymentForm = useForm({
        method: 'transfer',
        amount: order.outstanding_amount > 0 ? order.outstanding_amount.toString() : '',
        reference_number: '',
        notes: '',
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-4 rounded-[2rem] bg-white p-8 shadow-[0_20px_80px_rgba(31,23,38,0.08)] md:flex-row md:items-end md:justify-between">
                    ...
                    <Button asChild variant="outline" className="border-[#d8c8b3] bg-white/70">
                        <Link href={customer.orders.index()}>Kembali ke daftar order</Link>
                    </Button>
                </div>
                    ...
                    <div className="space-y-6">
                        <Card className="border-0 bg-white shadow-[0_16px_50px_rgba(31,23,38,0.06)]">
                            <CardHeader>
                                ...
                                <CardDescription>Informasi utama pesanan tailor.</CardDescription>
                            </CardHeader>
                            <CardContent className="grid gap-4 md:grid-cols-2">
                                <InfoRow label="Model garmen" value={order.garment_model?.name ?? '-'} />
                                <InfoRow label="Bahan" value={order.fabric?.name ?? '-'} />
                                <InfoRow label="Metode ukuran" value={order.measurement_mode ?? '-'} />
                                <InfoRow label="Measurement" value={order.measurement?.label ?? 'Offline / belum ada'} />
                                <InfoRow label="Target selesai" value={order.due_date ?? '-'} />
                                <InfoRow label="Catatan spesifikasi" value={order.spec_notes ?? '-'} />
                            </CardContent>
                        </Card>
                        ...
                        <Card className="border-0 bg-white shadow-[0_16px_50px_rgba(31,23,38,0.06)]">
                            <CardHeader>
                                ...
                                <CardDescription>Komponen harga yang disimpan di backend.</CardDescription>
                            </CardHeader>
                            <CardContent className="grid gap-3">
                                {order.items.map((item) => (
                                    <div key={item.id} className="rounded-2xl border border-[#eadfce] p-4">
                                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                            ...
                                            <div className="text-left md:text-right">
                                                <p className="text-sm text-slate-600">Harga satuan {formatCurrency(item.unit_price)}</p>
                        ...
                        <Card className="border-0 bg-white shadow-[0_16px_50px_rgba(31,23,38,0.06)]">
                            <CardHeader>
                                ...
                                <CardDescription>Semua pembayaran yang terkait dengan order ini.</CardDescription>
                            </CardHeader>
                            <CardContent className="grid gap-4">
                                {order.payments.map((payment) => (
                                    <div key={payment.id} className="rounded-2xl border border-[#eadfce] p-4">
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                {payment.proof_image_path && (
                                                    <p className="text-xs text-slate-500">
                                                        Bukti: {payment.proof_image_path}
                                                    </p>
                                                )}
                    ...
                    <div className="space-y-6">
                        <Card className="border-0 bg-[#1f1726] text-[#f5f1e8] shadow-[0_20px_80px_rgba(31,23,38,0.18)]">
                            ...
                            <CardContent className="space-y-4 text-sm">
                                ...
                                <SummaryRow label="Sudah dibayar" value={formatCurrency(order.paid_amount)} />
                                <div className="border-t border-white/15 pt-4">
                                    <SummaryRow label="Sisa tagihan" value={formatCurrency(order.outstanding_amount)} strong />
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        {order.outstanding_amount > 0 && (
                            <Card className="border-0 bg-white shadow-[0_16px_50px_rgba(31,23,38,0.06)]">
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Customer portal hanya menerima pembayaran transfer.
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="grid gap-4">
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(event) => paymentForm.setData('reference_number', event.target.value)}
                                        />
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(event) => paymentForm.setData('proof', event.target.files?.[0] ?? null)}
                                        />
                                        <InputError message={paymentForm.errors.proof} />
                                    </div>
                                    ...
                                    <Button type="button" disabled={paymentForm.processing} onClick={submitPayment}>
                                        Kirim pembayaran
                                    </Button>
                                </CardContent>
                            </Card>
...
function SummaryRow({
    ...
    return (
        <div className="flex items-center justify-between gap-4">
            ...
            <span className={strong ? 'text-lg font-semibold text-[#f5f1e8]' : 'font-medium text-[#f5f1e8]'}>
                {value}
            </span>
        </div>
    );
